...
面向开发者的雷鸟 iO 智能眼镜 SDK、手机 App 扩展与眼镜端应用研究项目，覆盖 **iOS、Android、原生鸿蒙 HarmonyOS**。从自有 AI、录音和 Agent 接入，进一步探索 **Turbo Display 自定义显示**与**眼镜端原生导航应用**。不是给小白直接安装的成品 App。
...
**这次不只是“把导航测试文字发到眼镜”，而是在眼镜端增加了独立的导航应用页面。** Turbo Display 继续负责自定义显示和传图，导航拥有自己的菜单入口、布局和运行状态，两者并存。

| 功能 | Turbo Display · 自定义显示 | 原生导航 · TNV1 |
| --- | --- | --- |
| 用来做什么 | 手机传图、自定义像素内容、显示能力研究 | 手机规划路线，眼镜原生绘制导航界面 |
...
| 适合什么场景 | 图片、原型界面、低频自定义显示；不是视频投屏 | 持续变化的导航指引，无需每次传一整张导航图片 |
...
**发布状态不要混淆：** 当前 main 已公开多端 SDK/App 扩展、R3 图片固件实验和手机后台导航修复；完整 **TDP1 / TNV1** 固件与配套手机协议代码尚未作为完整版本发布。现有 R3 Release **不包含**这里的 Turbo Display 和第九项导航，不能下载 R3 后照此功能介绍操作。功能实现与已发布文件的对应关系见[功能与版本说明](docs/TURBO_DISPLAY_NAVIGATION.md)。
...
整理于 **2026-09-22**；各端沿用各自已记录的验收结果，并非当天全量复测。下表比较已公开的四条 SDK/App 路线，**不把尚未完整发布的 TDP1/TNV1 固件应用算作各端默认能力**。
...
- [x] **眼镜端应用研究进展**：Turbo Display 自定义显示与独立原生导航页面已在研究设备实现并取得初步反馈，详见顶部功能区。
- [ ] **完整发布 TDP1 / TNV1**：整理匹配的源码、构建工具、固件校验和兼容性说明，完成脱敏及风险审查；不能用旧 R3 包代替。
