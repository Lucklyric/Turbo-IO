...
static NSString *ObservedSID,*OwnedSID,*Note=@"Waiting for official subtitle preview templates";
...
void TIONewsObserveSend(id plugin,NSDictionary *args){
    ...
    if(OwnedSID.length&&![sid isEqual:OwnedSID]){TIONewsCaptionStop();Note=@"Official subtitle session changed, News stopped";}
    if([e[@"type"] isEqual:@7]&&[j[@"scope"] isEqual:@"temporary"]&&[j[@"config"] isKindOfClass:NSDictionary.class]&&[j[@"config"][@"is_display"] isEqual:@YES]&&![j[@"force"] boolValue]){
        Plugin=plugin;BaseArgs=[args copy];Preview=[j copy];ObservedSID=sid;TextTemplate=nil;Note=@"Preview config observed, waiting for same-session text and stop templates";
        ...
        TextTemplate=[j copy];Note=@"Preview text template captured";
    }else if([sid isEqual:ObservedSID]&&[e[@"type"] isEqual:@3]){Stop=[j copy];Note=TextTemplate?@"Subtitle preview template ready. News still needs a check on the glasses":@"Preview has no text template. Phone reading only";}
}
void TIONewsObserveReceive(NSDictionary *event){
    ...
    if([e[@"type"] isEqual:@4]){TIONewsCaptionStop();Note=@"Audio upload detected, News stopped. Confirm official subtitles have ended";return;}
    if([e[@"type"] isEqual:@8]){BOOL ok=[j[@"code"] isEqual:@1]||[j[@"code"] isEqual:@2];Ready=ok;Note=ok?@"Preview session receipt OK. Glasses text not yet confirmed":@"Subtitle preview rejected, not forcing takeover";void(^done)(BOOL,NSString *)=[Started copy];Started=nil;if(done)done(ok,Note);if(!ok)TIONewsCaptionStop();}
}
...
void TIONewsCaptionStart(void (^completion)(BOOL,NSString *)){
    ...
    if(!Send(7,j)){TIONewsCaptionStop();completion(NO,@"Subtitle channel send failed");return;}
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW,10*NSEC_PER_SEC),dispatch_get_main_queue(),^{if(epoch==Epoch&&!Ready){void(^done)(BOOL,NSString *)=[Started copy];Started=nil;TIONewsCaptionStop();if(done)done(NO,@"Subtitle preview receipt timed out, stopped");}});
}
