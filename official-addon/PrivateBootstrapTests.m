...
int main(void){@autoreleasepool{
    NSMutableDictionary *j=[@{@"schema":@1,@"endpoint":@"https://example.com/v1/chat/completions",@"model":@"test-model",@"modelKey":@"synthetic-model-key",@"deepseekDisableThinking":@YES,@"voiceExitCommands":@YES,@"officialToken":@"must-not-import"} mutableCopy];
    NSData *(^encode)(void)=^{return [NSJSONSerialization dataWithJSONObject:j options:0 error:nil];};
    NSDictionary *valid=TIOPrivateBootstrapConfig(encode());assert(valid&&valid.count==5&&!valid[@"officialToken"]);
