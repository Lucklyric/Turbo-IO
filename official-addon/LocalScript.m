...
static NSString *State=@"No script loaded";
...
static void Show(void){
    ...
    if(text.length>160)[text setString:[text substringToIndex:160]];
    State=[NSString stringWithFormat:@"Page %lu of %lu",(unsigned long)i+1,(unsigned long)Pages.count];
    TIOLocalGlassesShowText(text.length?text:@"End of script");
}
void TIOLocalScriptStart(NSString *script){
    ...
    Pages=TIOPagePaginate(script,40);Heard=[NSMutableArray new];
