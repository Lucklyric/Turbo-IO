...
static void Run(NSString *scenario){Scenario=scenario;Models=Reads=Writes=PublicSearches=0;__block BOOL done=NO;__block NSString *error=nil;__block NSUInteger finals=0,cancelled=0;KnowledgeRequest *r=[KnowledgeRequest new];
    ...
    [r startEndpoint:[NSURL URLWithString:@"https://model.example/chat/completions"] key:@"synthetic-key" payload:TIOChatRequest(@"test",@"查项目")];NSDate *end=[NSDate dateWithTimeIntervalSinceNow:3];while(!done&&end.timeIntervalSinceNow>0)[NSRunLoop.mainRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:.01]];assert(done&&Writes==0&&PublicSearches==0);
